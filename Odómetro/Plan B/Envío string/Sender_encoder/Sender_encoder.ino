...
// Enviar el paquete indefinidamente hasta recibir una respuesta
  while (packetSize == 0) {
    LoRa.beginPacket();
    LoRa.print("1\n"); // Enviar "1" como cadena seguida de un salto de línea
    LoRa.endPacket();
...
// Leer el paquete recibido en un buffer binario
  ...
  buffer.toCharArray(configBuffer, sizeof(configBuffer));

  char *token;
      token = strtok(configBuffer, ",");
      if (token != NULL) {
          // Ignorar el primer token ("1")
          token = strtok(NULL, ",");
          if (token != NULL) {
              token = strtok(NULL, ",");
              if (token != NULL) {
                  encoderType = atoi(token);
                  token = strtok(NULL, "\n");
                  if (token != NULL) {
                      encoderRatio = atof(token);
...
void sendLoRaPacket() {
  ...
  String dataString = "2," + longToString(_LeftEncoderTicks) + "," + intToString(batteryLevel) + "\n";
 // Serial.println(dataString);
  // Send the data string character by character
  LoRa.beginPacket();
...
String longToString(long value) {
  char buffer[12];  // Enough space for a 32-bit signed long
  memset(buffer, 0, sizeof(buffer)); 
  ltoa(value, buffer, 10);  // Convert to ASCII string (base 10)
  return String(buffer);
}

// Function to convert an int value to a string
String intToString(int value) {
  char buffer[4];  // Enough space for a 16-bit signed int
  memset(buffer, 0, sizeof(buffer)); 
  itoa(value, buffer, 10);  // Convert to ASCII string (base 10)
  return String(buffer);
}